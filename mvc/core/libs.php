<?php
    ...
    function getCategoryHome($id_story){
        $conn = new dbCon();
        $conn = $conn->connect();
        
        try{
                $query = "SELECT * FROM story_category WHERE id_story = :id_story";
                $cmd = $conn->prepare($query);
                $cmd->bindValue(":id_story", $id_story);
                $cmd->execute();
    ...
    function getNameChapter($str){
        $arr = explode(':', $str);
        return $arr[0];
    }
    function getNumberChapter($str){
        $arr = explode('-', $str);
        return $arr[1];
    }

    function checkChapNext($id_story,$number_chapter){
        $conn = new dbCon();
        $conn = $conn->connect();
        try{
